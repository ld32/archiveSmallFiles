...
for i in {1..10}; do 
    ...
    find "$sDir" -mindepth $i -maxdepth $i -type d 2>> $tempFile.0.err | while read -r dir; do  
      printf "%s\n" "Working on $dir"
...
xargs -0 -P "$nProc" -I "{}" bash -c '
    ...
    find "$2" -type d -print0 2>> "$errFile" | while IFS= read -r -d "" dir; do
...
count=$(find "$sDir" -maxdepth 1 -type f -o -type l | wc -l)
